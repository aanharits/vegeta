...
export default function Products({ params }: { params: { id: string } }) {
  ...
  const { data: productDetails, isLoading: isLoadingProduct } = useGetProductByIdQuery(params.id);
  const { data: recommendedProducts, isLoading: isLoadingRecommended } = useGetAllProductsQuery({});
  ...
  const handleAddToCart = async () => {
    ...
    toast({
      title: "Berhasil!",
      description: `${productDetails?.data.name} telah ditambahkan ke keranjang`,
      variant: "default",
      duration: 2000,
    })
  }
  ...
  return (
    <main className="flex flex-col w-full min-h-screen items-center pb-8">
      {isLoadingProduct ? (
        ...
        <div className="w-content flex pt-5 gap-12 animate-fadeIn">
